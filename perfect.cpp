...
long smarterSum(long n) {
    long total = 0;
    for (long num = 1; num < sqrt(n); num++){
        if (n % num == 0 && n!=num){
            total += num;
                if (n/num != num && n!=n/num){
                total +=n/num;
            }
        }
    }
    cout << "Found perfect number: "  << endl;
    return total;
}
