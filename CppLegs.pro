...
###############################################################################
#       Autocommit hook per-OS (no chmod needed)
###############################################################################
# pass repo root, target name, and destdir to both variants.

win32 {
   CONFIG += console
   QMAKE_POST_LINK += powershell -ExecutionPolicy Bypass -NoProfile -File $$shell_quote($$PWD/auto_commit.ps1) $$shell_quote($$PWD) $$TARGET $$shell_quote($$DESTDIR) || ver>nul


}else {
    BASH      = /bin/bash
    SH_SCRIPT = $$shell_quote($$PWD/auto_commit.sh)
    QMAKE_PRE_LINK += $$BASH $$SH_SCRIPT \
        $$shell_quote($$PWD) \
        $$shell_quote($$TARGET) \
        $$shell_quote($$DESTDIR) \
        || true
